...
#import "EOSFTPServerFile+Private.h"
#import <sys/stat.h>

@implementation EOSFTPServerFile( Private )
...
- ( BOOL )getFileInformations
{
    ...
    if( stats.st_flags & SF_ARCHIVED  ) { _flags |= EOSFTPServerFileFlagsArchived; }
    if( stats.st_flags & UF_HIDDEN    ) { _flags |= EOSFTPServerFileFlagsHidden; }
    if( stats.st_flags & UF_NODUMP    ) { _flags |= EOSFTPServerFileFlagsNoDump; }
    if( stats.st_flags & UF_OPAQUE    ) { _flags |= EOSFTPServerFileFlagsOpaque; }
    if( stats.st_flags & SF_APPEND    ) { _flags |= EOSFTPServerFileFlagsSystemAppendOnly; }
    if( stats.st_flags & SF_IMMUTABLE ) { _flags |= EOSFTPServerFileFlagsSystemImmutable; }
    if( stats.st_flags & UF_APPEND    ) { _flags |= EOSFTPServerFileFlagsUserAppendOnly; }
    if( stats.st_flags & UF_IMMUTABLE ) { _flags |= EOSFTPServerFileFlagsUserImmutable; }
    ...
    /* File type */
         ...
         if( [ attributes objectForKey: NSFileType ] == NSFileTypeDirectory        ) { _type = EOSFTPServerFileTypeDirectory;        }
    else if( [ attributes objectForKey: NSFileType ] == NSFileTypeRegular          ) { _type = EOSFTPServerFileTypeRegularFile;      }
    else if( [ attributes objectForKey: NSFileType ] == NSFileTypeSymbolicLink     ) { _type = EOSFTPServerFileTypeSymbolicLink;     }
    else if( [ attributes objectForKey: NSFileType ] == NSFileTypeSocket           ) { _type = EOSFTPServerFileTypeSocket;           }
    else if( [ attributes objectForKey: NSFileType ] == NSFileTypeCharacterSpecial ) { _type = EOSFTPServerFileTypeCharacterSpecial; }
    else if( [ attributes objectForKey: NSFileType ] == NSFileTypeBlockSpecial     ) { _type = EOSFTPServerFileTypeBlockSpecial;     }
    else if( [ attributes objectForKey: NSFileType ] == NSFileTypeUnknown          ) { _type = EOSFTPServerFileTypeUnknown;          }
    else                                                                             { _type = EOSFTPServerFileTypeUnknown;          }
